...
internal static class Program
{
	...
	private const int MAX_TURNFRAMES = 70;
	...
	private static void ThreadProc(object? param)
	{
		...
		{
			SearchKeyInput(i, false, false);
			SearchKeyInput(i, false, true);
			SearchKeyInput(i, true, false);
			SearchKeyInput(i, true, true);
		}
	}
	...
	private static bool CheckEncounter(ref ulong pidRng)
	{
		...
		if (encounterCheck < 4)
		{
			...
			// we have usually repel, so we don't particularly care this happens
	...
	private static (ulong PidRng, int Steps)? CheckDustCloudNoRepel(ulong pidRng, int numInitialTurnFrames, int nunLatterTurnFrames, int stepsToNextEncounterTile,
	...
	{
		for (var i = 0; i < numInitialTurnFrames; i++)
		{
			// apparently has some immunity on the first tile?
			AdvancePidRng(ref pidRng);
		}

		for (var i = 0; i < 19; i++)
		{
			if (CheckEncounter(ref pidRng))
			{
				return null;
			}
		}

		for (var i = 0; i < nunLatterTurnFrames; i++)
		{
			if (CheckEncounter(ref pidRng))
			{
				return null;
		...
		for (var i = 0; i < stepsToNextEncounterTile; i++)
		{
			if (CheckEncounter(ref pidRng))
			{
				return null;
		...
		for (var i = 0; i < MAX_TURNFRAMES; i++)
		{
			if (CheckEncounter(ref pidRng))
			{
				for (var j = 0; j < 3; j++)
				{
					AdvancePidRng(ref pidRng); // 3 other pidrng rolls for actually successful encounter
				}

				stepsToNextEncounter = i;
				break;
			}
		}

		if (stepsToNextEncounter == -1)
		{
			return null;
		}

		return (pidRng, stepsToNextEncounter);
	}
	...
	private static void CheckSeedPathSkip6(ulong pidRng, string path12345, List<string> results)
	{
		...
		for (var i = 0; i < MAX_TURNFRAMES; i++)
		{
			...
			{
				results.Add($"{path12345} A{i} | End Seed {nextPidRngA.Value:X016}");
			}
			...
			{
				results.Add($"{path12345} B{i} | End Seed {nextPidRngB.Value:X016}");
			}
			...
			{
				results.Add($"{path12345} C{i} | End Seed {nextPidRngC.Value:X016}");
			}
			...
			{
				results.Add($"{path12345} D{i} | End Seed {nextPidRngD.Value:X016}");
			}
			...
			{
				results.Add($"{path12345} E{i} | End Seed {nextPidRngE.Value:X016}");
			}
			...
			{
				results.Add($"{path12345} F{i} | End Seed {nextPidRngF.Value:X016}");
			}
		}
	}
	...
	private static void CheckSeedPathSkip5(ulong pidRng, string path1234, List<string> results)
	{
		...
		for (var i = 0; i < MAX_TURNFRAMES; i++)
		{
			...
			if (nextPidRngA.HasValue)
			{
				CheckSeedPathSkip6(nextPidRngA.Value, $"{path1234} A{i}", results);
			}
			...
			if (nextPidRngB.HasValue)
			{
				CheckSeedPathSkip6(nextPidRngB.Value, $"{path1234} B{i}", results);
			}
			...
			if (nextPidRngC.HasValue)
			{
				CheckSeedPathSkip6(nextPidRngC.Value, $"{path1234} C{i}", results);
			}
			...
			if (nextPidRngD.HasValue)
			{
				CheckSeedPathSkip6(nextPidRngD.Value, $"{path1234} D{i}", results);
			}
		}
	}
	...
	{
		// rng advances once after running away from wild battle (for some reason)
		AdvancePidRng(ref pidRng);
		...
		for (var i = 0; i < MAX_TURNFRAMES; i++)
		{
			...
			if (nextPidRngE.HasValue)
			{
				CheckSeedPathSkip5(nextPidRngE.Value, $"{path123} E{i}", results);
			}
		}
	}
	...
	private static void CheckSeedPathSkip3(ulong pidRng, string path12, List<string> results)
	{
		...
		for (var i = 0; i < MAX_TURNFRAMES; i++)
		{
			for (var j = 0; j < MAX_TURNFRAMES; j++)
			{
				// nearest tile (56, 16)
				var nextPidRngASteps = CheckDustCloudNoRepel(pidRng, i, j, 2,
					numRightwardsSlots: 0x0A, wantedRightwardsSlots: [1, 2],
					numLeftwardsSlots: 0x27, wantedLeftwardsSlots: [11, 17],
					numUpwardsSlots: 0x18, wantedUpwardsSlots: [11, 17],
					numDownwardsSlots: 0, wantedDownwardsSlots: []);

				if (nextPidRngASteps.HasValue)
				{
					var (nextPidRng, stepsToSecondEncounter) = nextPidRngASteps.Value;
					CheckSeedPathSkip4(nextPidRng, $"{path12} A{stepsToNextEncounter}/{i}/{j}/{stepsToSecondEncounter}", results);
				}

				// 1 tile away (56, 17)
				var nextPidRngBSteps = CheckDustCloudNoRepel(pidRng, i, j, 3,
					numRightwardsSlots: 0x0A, wantedRightwardsSlots: [1, 2],
					numLeftwardsSlots: 0x27, wantedLeftwardsSlots: [11, 17],
					numUpwardsSlots: 0x19, wantedUpwardsSlots: [11, 17],
					numDownwardsSlots: 0, wantedDownwardsSlots: []);

				if (nextPidRngBSteps.HasValue)
				{
					var (nextPidRng, stepsToSecondEncounter) = nextPidRngBSteps.Value;
					CheckSeedPathSkip4(nextPidRng, $"{path12} B{stepsToNextEncounter}/{i}/{j}/{stepsToSecondEncounter}", results);
				}

				// 2 tiles away (56, 18)
				var nextPidRngCSteps = CheckDustCloudNoRepel(pidRng, i, j, 4,
					numRightwardsSlots: 0x09, wantedRightwardsSlots: [0, 1],
					numLeftwardsSlots: 0x21, wantedLeftwardsSlots: [5, 11],
					numUpwardsSlots: 0x14, wantedUpwardsSlots: [5, 11],
					numDownwardsSlots: 0, wantedDownwardsSlots: []);

				if (nextPidRngCSteps.HasValue)
				{
					var (nextPidRng, stepsToSecondEncounter) = nextPidRngCSteps.Value;
					CheckSeedPathSkip4(nextPidRng, $"{path12} C{stepsToNextEncounter}/{i}/{j}/{stepsToSecondEncounter}", results);
				}

				// 3 tiles away (56, 19)
				var nextPidRngDSteps = CheckDustCloudNoRepel(pidRng, i, j, 5,
					numRightwardsSlots: 0x08, wantedRightwardsSlots: [0],
					numLeftwardsSlots: 0x1B, wantedLeftwardsSlots: [5],
					numUpwardsSlots: 0x0F, wantedUpwardsSlots: [5],
					numDownwardsSlots: 0, wantedDownwardsSlots: []);

				if (nextPidRngDSteps.HasValue)
				{
					var (nextPidRng, stepsToSecondEncounter) = nextPidRngDSteps.Value;
					CheckSeedPathSkip4(nextPidRng, $"{path12} D{stepsToNextEncounter}/{i}/{j}/{stepsToSecondEncounter}", results);
				}
			}
		}
	}
	...
	private static void CheckSeedPathSkip2(ulong pidRng, string path1, List<string> results)
	{
		...
		for (var i = 0; i < MAX_TURNFRAMES; i++)
		{
			...
			var nextPidRngB = CheckDustCloud(pidRng, i, 2,
	...
	private static List<string> CheckSeedPathSkip1(ulong pidRng)
	{
	...
	private static void SearchKeyInput(int keyInput, bool closeLid, bool earlyTimer0)
	{
		...
		initData[8 * 4 + 0] = _bcdValues[77];
		initData[8 * 4 + 1] = _bcdValues[8];
		initData[8 * 4 + 2] = _bcdValues[31];
		initData[8 * 4 + 3] = ComputeWeekday(77, 8, 31);
		// [9] is time, in HH MM SS 00 format
		initData[9 * 4 + 0] = (byte)(_bcdValues[23] | 0x40);
		initData[9 * 4 + 1] = _bcdValues[37];
		initData[9 * 4 + 2] = _bcdValues[18];
		initData[9 * 4 + 3] = 0x00;
		...
		static int ParseDelayFromResult(string result)
		{
			...
			var delay = strs.Sum(str =>
			{
				if (str.Contains('/', StringComparison.Ordinal))
				{
					var innerStrs = str[1..].Split('/');
					return innerStrs.Sum(int.Parse);
				}
...

		{
			...
			var resultsByDelay = new List<(int, string)>();
			foreach (var result in results)
			{
				var delay = ParseDelayFromResult(result);
				if (delay > MAX_TURNFRAMES)
				{
					continue;
				}

				resultsByDelay.Add((delay, $"Delay {delay} | Path {result} | Timer0 {timer0:X02} | Input {(KeyInput)keyInput} | Seed: {seed:X016}"));
			}

			resultsByDelay.Sort((delayX, delayY) => delayX.Item1.CompareTo(delayY.Item1));

			foreach (var (_, result) in resultsByDelay)
			{
